...
class Concierge_Latest_posts extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'concierge_widget_recent_entries', 'description' => __( "Your site&#8217;s Latest Posts.",'concierge' ) );
        parent::__construct('concierge-recent-posts', __('concierge Latest Posts','concierge' ), $widget_ops);
    ...
    public function widget($args, $instance) {
        ...
        $title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Latest Posts','concierge'  );
            ...
            <?php echo apply_filters( 'before_recent_post_widget',  $args['before_widget'] ); ?>
                ...
                <ul class="custom-list list recent-post-list">
                    <?php while ( $r->have_posts() ) : $r->the_post(); ?>
                        ...
                        <li><a href="<?php the_permalink(); ?>"> <?php  echo esc_attr(get_the_title());  ?></a></li>                    
    ...
    ?>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e( 'Number of posts to show:','concierge'  ); ?></label>
            <input id="<?php echo esc_attr($this->get_field_id('number')); ?>" name="<?php echo esc_attr($this->get_field_name('number')); ?>" type="text" value="<?php echo esc_attr($number); ?>" size="3" />
        </p>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id('words')); ?>"><?php _e( 'Words of words to be displayed:','concierge'  ); ?></label>
...
add_action('widgets_init', 'concierge_latest_posts');

function concierge_latest_posts(){

    register_widget('Concierge_Latest_posts');
