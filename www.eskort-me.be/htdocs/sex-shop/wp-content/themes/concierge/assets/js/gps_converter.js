...
function loader() {
    ...
    var marker;
    var zoom;
    var start_lat = jQuery("#_concierge_property_address_lat").val();
    var start_lng = jQuery("#_concierge_property_address_lng").val();
    ...
    google.maps.event.addListener(marker, 'drag', function() {
        $("#_concierge_property_address_lat").val(marker.getPosition().lat());
        $("#_concierge_property_address_lng").val(marker.getPosition().lng());
    });
        ...
        geocoder.geocode({latLng: marker.getPosition(), region: 'en' }, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                if (results) {
                    ...
                    $("#_concierge_property_address_country_name").val(" ");
                    $("#_concierge_property_address_region_name").val(" ");
                    $("#_concierge_property_address_name").val(" ");
                    $("#_concierge_property_address_zip").val(" ");
                    for (var index_res=0; index_res <= (results.length - 1); index_res++) {
                        ...
                        for (var index_count=0; index_count <= (results[index_res].address_components.length - 1); index_count++) {
                            ...
                            if ((results[index_res].address_components[index_count].types[0] == "country") && (marker_country == false)) {
                                $("#_concierge_property_address_country_name").val(results[index_res].address_components[index_count].long_name);
                                marker_country = true;
                            }
                            if ((results[index_res].address_components[index_count].types[0] == "administrative_area_level_1") && (marker_region == false)) {
                                $("#_concierge_property_address_region_name").val(results[index_res].address_components[index_count].long_name);
                                marker_region = true;
                            }
                            ...
                            if ((results[index_res].address_components[index_count].types[0] == "postal_code") && (marker_zip == false)) {
                                $("#_concierge_property_address_zip").val(results[index_res].address_components[index_count].long_name);
                                marker_zip = true;
                            }
                        }
                    }
                    for (index_count=0; index_count <= (results[0].address_components.length - 1); index_count++) {
                        if ((results[0].address_components[index_count].types[0] != "country") &&
                            ...
                            if ($("#_concierge_property_address_name").val() == " ") { $("#_concierge_property_address_name").val(results[0].address_components[index_count].long_name); }
                            else { $("#_concierge_property_address_name").val($("#_concierge_property_address_name").val() + ", " + results[0].address_components[index_count].long_name); }
                        }
                    }
...
$('#_concierge_property_address_convert_zip').click(function(){

     var geocoder, geocoder_drag;
    var map;
    var start_lat = jQuery("#_concierge_property_address_lat").val();
    var start_lng = jQuery("#_concierge_property_address_lng").val();
    var country = $("#_concierge_property_address_country_name").val();
    var region = $("#_concierge_property_address_region_name").val();
    var address = $("#_concierge_property_address_name").val();
    var zip = $("#_concierge_property_address_zip").val();
    ...
    geocoder.geocode( { address: full_address, region: 'en'}, function(results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            $('#_concierge_property_address_lat').val(results[0].geometry.location.lat());
            $('#_concierge_property_address_lng').val(results[0].geometry.location.lng());
