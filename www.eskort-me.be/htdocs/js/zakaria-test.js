...
(function($){
	...
	/*navigator.geolocation.getCurrentPosition(function maPosition(position){
		  ...
		  $("#position_user").val(user); 
		  alert("test"+$("#position_user").val());
		});
	});
	alert("apres "+$("#position_user").val());*/
	function get(nb){
	...
	function createNode(data,i){
		...
		/*$.ajax({
			...
			type:'POST',
			dataType:'json',
			data:'&action=filemtime&photo=photos/'+data.id_fille+data.nom+'/'+data.id_fille+data.nom+'1.png',
			success:function(retour_php)
			{
				filemtime = retour_php;
			},
			async: false
		});*/
		...
		html += '<div class="thumb fleet-thumb">';
			...
			html += '<div class="overlay">';
				...
				html += '<img src="photos/'+data.id_fille+data.nom+'/'+data.id_fille+data.nom+'1.png?v='+data.fileMtime+'" alt="Massage tantrique '+data.nom+'" width="270" height="390" />';
				
				html += '<div class="overlay-shadow"><div class="overlay-content">';
				
				html += '<a href="'+data.link+'" class="btn light" title="Massage tantrique '+data.nom+'" onClick="compteurFille('+data.id_fille+')">'+data.nom+'</a>';
			...
			html += '<div class="fleet-vechicle-content">';
				...
				//html += '<span>Distance : '+parseInt(distance/1000)+' km</span>';
