...
//si le nombre de resultat n'est pas en session on l'y place, il peut etre modifi� par la suite
...
//on regarde dans element si des artciles sont retourn�s, si pas, on va afficher un choix au hasard(ou pas)
// d'articles
$article_present = 0;
...
foreach ($element as $values) {
    ...
    if ($values->getType_element() == 'art') {

        $article_present = 1;
    } elseif ($values->getType_element() == 'cat') {
...
if ($categorie_presente == 1) {
    ...
    foreach ($element as $values) {
        ...
        if ($values->getType_element() == 'cat') {



            // si on a des categories
            $lien = Utilitaires::lien('2', $lang, $site, $ariane, $values->getId_element(), $values->getNom_element());
            ...
            echo'<p>';
            echo'<a href="./' . $lien . '">' . $values->getNom_element() . '</a>';
            echo'</p>';
...
if (!isset($element)) {
    ...
    if ($article_present == 0) {
        ...
        $element2 = ElementFactory::ElementSelectSSCat($_SESSION['lang'], 0, $_GET['e'], 0, 6);
...
$lien_art = Utilitaires::lien('21', $lang, $site, $ariane, $values->getId_element(), $values->getNom_element());
                //on affiche un lot d'images suivant un choix pr�defini
                ...
                echo'<img style="padding-top:' . $margintop . 'px;" src="fonctions/fly_thumb.php?links=' . $lien_photo[0] . '&wm=140&w=' . $taille_max_img_finie . '&h=' . $taille_max_img_finie . '" alt="' . $nom_photo . '" border="0"></a>';
                echo'</div>';
                echo'<p>';
                $prixV=$values->getPrix_vente();
                $prixL=$values->getPrix_loc();
                
                echo'<a href="./' . $lien_art . '">' . $values->getNom_element() . '</a></p>';
                //si prix de vente different de 0 on affiche idem pour loc
                if(!is_null($prixV) and $prixV!='0' and $prixV!='0'){
                   echo'<p><a href="./' . $lien_art . '">'. $prixV[0]['montant'] . ' &euro; ' . $values->getUnite . '</a></p>';
                }
                if(!is_null($prixV) and $prixL!='0' and $prixL!=''){
                    echo'<p><a href="./' . $lien_art . '">'.$lang_prixloc[$lang].' '. $prixL[0]['montant'] . ' &euro; ' . $values->getUnite . '</a></p>';
}
...
$lien_art = Utilitaires::lien('21', $lang, $site, $ariane, $values->getId_element(), $values->getNom_element());
 ...
 $lien_photo=$values->getLien_photo() ;
                ...
                if ($lien_photo[0] == 'vide.jpg') {
                    ...
                    if(isset($nom_image))
                        $size = getimagesize('./photo/element/' . utf8_decode($nom_image) . '.thumb' . $ext_image);
                }
                $height_base = $size[1];
                $ratio = $taille_max_img_finie / $size[0];
                ...
                echo'<a href="./' . $lien_art . '">' . $values->getNom_element() . '</a></p>';
                  $prixV=$values->getPrix_vente();
                $prixL=$values->getPrix_loc();
                //si prix de vente different de 0 on af
                //fiche idem pour loc
                if(!is_null($prixV) and $prixV!='0' and $prixV!=''){
                   echo'<p><a href="./' . $lien_art . '">'. $prixV[0]['montant'] . ' &euro; ' . $values->getUnite . '</a></p>';
                }
                if(!is_null($prixV) and $prixL!='0' and $prixL!=''){
                    echo'<p><a href="./' . $lien_art . '">'.$lang_prixloc[$lang].' '. $prixL[0]['montant'] . ' &euro; ' . $values->getUnite . '</a></p>';
...
/*           $modele = ElementFactory:: ElementSelectFREE($lang, 0, 'element.type=\'modele\'');
  ...
  if ($limit_inf > 0) {
  echo'<a style="color:#006699" href="' . $links[0] . '?limit_inf=' . $limit_inf_moins . $ifrecherche . '">pr�c�dent </a>';
  }
