...
foreach ($panier->article as $ref_article => $valeurs) {
    ...
    if (!is_null($elements)) {
        foreach ($elements as $element) {
            $type = $element->getStyle();
            if($type==='reel') { $all++; } else {$_SESSION['download']++;}
        ...
        echo '<td id="nom"><p>' . $nom_element . '<br><br>' . $nom_model." ".$type;
...
echo'</tr>';
$_SESSION['prix_total_stripe']=$prix_total;
$prix_stripe = $prix_total;
if (isset($_SESSION['id_client'])) {
...
if (!empty($panier->article)) {//si le panier est vide pas de bouton
    ...
    echo'<form action="./fr-pg103-' . $site . '.html" method="post">';
    ...
    if ($erreur_traitement != 1) {
        ...
        if (isset($_SESSION['id_client'])) {
          if($all>=1)
          {
            echo'<input type="image" src="./img/valider_commande.png"  name="submit" alt="' . $lang_valider[$lang] . '" title="' . $lang_valider[$lang] . '">';
          }
          elseif($all==0) {
            echo'<a href="./fr-pg72-' . $site . '.html"><img src="./img/valider_commande.png" alt="' . $lang_valider[$lang] . '" title="' . $lang_valider[$lang] . '"></a>';
          }
        } else {
